var SimpleGmailNotes_Gmail = function(localJQuery) {
  ...
  */
        ...
        try{
          ...
          if(api.observe.bound(events,'after')) {
            ...
            this.onreadystatechange = function(progress) {
              //console.log("@896", this.responseText.substring(0, 500));
              if (this.readyState === this.DONE) {
                ...
                if(this.responseText.indexOf('var GM_TIMING_START_CHUNK2') == 0){
                    var dummy="view_data";
                }
                else if(this.responseText.indexOf(")]}") == 0){
                    var dummy="json";
                }
              }
          ...
          //walty: in any case, do not harm the original request sending
          try{
            addErrorToLog(err)
          }
          catch(err2){
  ...
  //new API by walty
  api.tools.parse_json_data = function(email_data) {
    var data = {};
    var threads = {}
    var parsed_data = {};
    ...
    for(i in email_data) {
      ...
      if(x[0] == 'ms') {    //message
        ...
        var id = x[1];
        data.threads[id] = {};
        data.threads[id].is_deleted = x[13] == undefined;
        data.threads[id].reply_to_id = x[2];
        data.threads[id].from = x[5];
        data.threads[id].from_email = x[6];
        data.threads[id].timestamp = x[7];
        data.threads[id].datetime = x[24];
        data.threads[id].attachments = x[21].split(',');
        data.threads[id].subject = x[12];
        data.threads[id].content_html = (x[13] != undefined) ? x[13][6] : x[8];
        data.threads[id].to = (x[13] != undefined) ? x[13][1] : ((x[37] != undefined) ? x[37][1]:[]);
        data.threads[id].cc = (x[13] != undefined) ? x[13][2] : [];
        data.threads[id].bcc = (x[13] != undefined) ? x[13][3] : [];
        data.threads[id].reply_to = api.tools.get_reply_to(x[13]);

        try { // jQuery will sometime fail to parse x[13][6], if so, putting the raw HTML
          data.threads[id].content_plain = (x[13] != undefined) ? $(x[13][6]).text() : x[8];
        }
        catch(e) {
          data.threads[id].content_plain = (x[13] != undefined) ? x[13][6] : x[8];
        }
      }
    ...
    for(var i=0; i<email_list.length; i++){
      ...
      email.sender = data.threads[email.last_email].from_email;
      email.time = data.threads[email.last_email].datetime;
      email.title = data.threads[email.first_email].subject;
    }
